	...
	public static void main(String args[])
	{
		...
		try
		{
			...
			*/
			...
			int type;
			int index;
			HashMap<String,Integer> relevantURLs=null;
			...
			while (true)
			{
				...
				if(splitedstr[1].equals("M"))
				{
					...
					if(currentUserID!=userId)
					{
						...
						relevantURLs=new HashMap<String,Integer>();
					}
					...
					for(int i=6;i<splitedstr.length;i++)
						{
							...
							if(relevantURLs.containsKey(urldomainsplit[0]+","+queryID))
							{
								int rel=relevantURLs.get(urldomainsplit[0]+","+queryID);
							...
							{
								relevantURLs.put(urldomainsplit[0]+","+queryID, 0);
							}
					...
					while ((str = bfrread.readLine()) != null)
					{
						...
						if(splitedstr[2].equals("C"))
						{
							...
							if(type==1)
								...
								relevantURLs.put(urlid+","+queryID, 3);
							...
							if(type==2)
								...
								if(relevantURLs.get(urlid+","+queryID)<2)
									relevantURLs.put(urlid+","+queryID, 2);
								//rwtperquery[index].delete(0, rwtperquery[index].length());
							...
							if(type==3)
								...
								if(relevantURLs.get(urlid+","+queryID)<1)
									relevantURLs.put(urlid+","+queryID, 1);
								//rwtperquery[index].delete(0, rwtperquery[index].length());
